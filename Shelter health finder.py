...
try:
    ...
    print("Please install the cryptography module ('pip install cryptography')")
    quit()
...
def decrypt(filestring):
        cipherBits = base64.decodestring(filestring.encode("utf-8"))

        backend = default_backend()
        key = bytes(bytearray.fromhex("A7CA9F3366D892C2F0BEF417341CA971B69AE9F7BACCCFFCF43C62D1D7D021F9"))
        iv = bytes(bytearray.fromhex("7475383967656A693334307438397532"))
        decryptor = Cipher(algorithms.AES(key), modes.CBC(iv), backend=backend).decryptor()
        
        decr = decryptor.update(cipherBits).decode("utf-8")
        decr = decr.replace(chr(12),'')
        return decr



print("opening file")
file =  open("Vault.sav", "r")
decryptedfile = decrypt(file.read())

fileobj = json.loads(decryptedfile)
        ...
        if commandList[0].strip().lower() == "sort":
                ...
                if (tempSortIndex < 1) or (tempSortIndex > len(DwellerFields)):
                    ...
                    if len(commandList) > 2:
                        SortReversed = (commandList[2].strip().lower() == "-r")
                    ListDwellerInfo(DwellerFields,  DwellerFields[SortIndex - 1], SortReversed)

        if commandList[0].strip().lower() == "quit":
            quit()
                #print(tempSortIndex, commandList, SortReversed)
